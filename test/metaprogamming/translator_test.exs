defmodule MetaProgramming.TranslatorTest do
  use ExUnit.Case
  ...
  test "compile/1 generates catch-all t/3 functions" do
    assert MetaProgramming.Translator.compile([]) |> Macro.to_string() ==
             String.trim(~S"""
  ...
  test "compile/1 generates t/3 functions from each locale" do
    ...
    assert MetaProgramming.Translator.compile(locales) |> Macro.to_string() ==
             String.trim(~S"""
             def t(locale, path, binding \\ [])
             ...
             [
               [
                 ...
                 def t("en", "bar", bindings) do
                   (("" <> "") <> to_string(MetaProgramming.Translator.lookup!(bindings, :baz))) <> ""
                 end
               ]
             ]
